export default function Select({
    ...
    const handleInvalid = (e: React.FormEvent<HTMLSelectElement>) => {
        ...
        select.classList.add('!border-red-400', '!focus:border-red-400', 'focus:ring-red-400');
        const feedback = e.currentTarget.nextElementSibling as HTMLElement;
        feedback.textContent = select.validationMessage;
    };

    const handleChange = (e: React.FormEvent<HTMLSelectElement>) => {
        const select = e.target as HTMLSelectElement;
        select.classList.remove('!border-red-400', '!focus:border-red-400', 'focus:ring-red-400');
        const feedback = e.currentTarget.nextElementSibling as HTMLElement;
